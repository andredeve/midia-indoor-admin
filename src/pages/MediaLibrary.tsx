...
export default function MediaLibrary() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type === 'video/webm') {
      alert('⚠️ ATENÇÃO: O formato .webm não é suportado por iPhones/iPads. Por favor, converta para .mp4 antes de enviar para garantir que a playlist funcione em todos os aparelhos.');
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          ...
          >
            ...
            {uploading ? 'Enviando...' : 'Subir Vídeo/Imagem (.mp4)'}
          </button>
        </div>
      </div>

      <div className="glass-panel" style={{ padding: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '12px', border: '1px solid rgba(255, 193, 7, 0.2)' }}>
        <AlertTriangle color="#ffc107" size={20} />
        <p style={{ fontSize: '0.875rem', color: '#ffc107' }}>
          <strong>Dica:</strong> Use sempre o formato <strong>.mp4</strong> para vídeos. Arquivos .webm não rodam em aparelhos Apple.
        </p>
      </div>
              ...
              <div style={{ height: '140px', backgroundColor: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                {item.file_type.includes('video') ? (
                  <>
                    ...
                    {item.file_type === 'video/webm' && (
                      <div style={{ position: 'absolute', bottom: 5, right: 5, background: '#ff4444', color: 'white', fontSize: '10px', padding: '2px 4px', borderRadius: '4px' }}>WEBM</div>
                    )}
                  </>
                ) : (
